...
# This code is running on each instance right now. Changing to use function start() to run.
# start() will recursively call itself to restart the server on a disconnect exception.
# while True:
#     try:
#         asyncio.run(run())
#     except websockets.ConnectionClosedError:
